...
1. **循环依赖**：Buffer 引用自己会自动启用双缓冲，读到的是自己上一帧的结果。多个 Buffer 互相引用（A↔B，或更长的环如 A→B→C→A）**不需要**双缓冲也不会冲突——各通道读写的是不同纹理，按 `passes` 顺序依次渲染即可；代价是环里第一个渲染的通道读到的是上一帧，其余读到本帧，所以环的入口写在哪里会直接影响结果
